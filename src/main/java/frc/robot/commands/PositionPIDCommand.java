...
public class PositionPIDCommand extends Command {
    ...
    private final DoublePublisher xErrLogger = NetworkTableInstance.getDefault()
            .getTable("logging")
            .getDoubleTopic("X Error")
            .publish();
    private final DoublePublisher yErrLogger = NetworkTableInstance.getDefault()
            .getTable("logging")
            .getDoubleTopic("Y Error")
            .publish();
    ...
    private PositionPIDCommand(CommandSwerveDrivetrain mSwerve, Pose2d goalPose) {
        ...
        mDriveController =
                new PPHolonomicDriveController(new PIDConstants(5.5, 0.0, 0.1, 0.0), new PIDConstants(5, 0, 0));
    }
    ...
    public void execute() {
        ...
        mSwerve.setControl(robotSpeeds.withSpeeds(
                mDriveController.calculateRobotRelativeSpeeds(mSwerve.getState().Pose, goalState)));

        xErrLogger.accept(mSwerve.getState().Pose.getX() - goalPose.getX());
        yErrLogger.accept(mSwerve.getState().Pose.getY() - goalPose.getY());
        Logger.recordOutput("X Error", mSwerve.getState().Pose.getX() - goalPose.getX());
        Logger.recordOutput("Y Error", mSwerve.getState().Pose.getY() - goalPose.getY());
    }
    ...
    public boolean isFinished() {
        ...
        var speed = Math.sqrt(Math.pow(mSwerve.getState().Speeds.vx, 2) + Math.pow(mSwerve.getState().Speeds.vy, 2))
                < InchesPerSecond.of(2).in(MetersPerSecond);

        // System.out.println("end trigger conditions R: "+ rotation + "\tP: " + position + "\tS: " + speed);

        return endTriggerDebouncer.calculate(rotation && position && speed);
    }
}
